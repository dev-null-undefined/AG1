...
struct Tree {
    ...
    const T *find(const T &value) const {
        auto it = _tree.find(value);
        if (!it) return nullptr;
        return &it->getValue();
    }

    bool insert(T value) {
        return _tree.insert(std::move(value));
    }
    ...
    using NodeType = typename AVLTree<T>::NodeType;
    ...
    struct TesterInterface {
        // using Node = ...
        static const NodeType *root(const Tree *t) { return t->_tree.root; }
